...
// Don't define custom interfaces, use Prisma's generated types instead
  ...
  try {
    ...
    try {
      ...
      formattedEntries = entries.map((entry) => {
        ...
        const transactions = entry.transactions?.map(txn => ({
          ...txn,
        ...
        return {
          id: entry.id.toString(), // Convert to string for consistency with sample data
          ...
          party: entry.vendor?.name || entry.customer?.name || "",
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract manual vendor/customer name from notes if applicable
    const extractPartyName = (notes: string | null, prefix: string) => {
      if (!notes || !notes.includes(`${prefix}:`)) return null;
